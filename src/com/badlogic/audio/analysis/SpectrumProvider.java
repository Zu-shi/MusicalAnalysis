...
public class SpectrumProvider 
{
	...
	{		
		...
		if( currentSample >= samples.length )
		{
			...
			nextSamples = samples; //Overwrite the nextsamples with samples, while keeping nextsamples in temp.
			samples = tmp; //Samples is now nextsamples, and nextsamples take care of the new samples. 
			if( decoder.readSamples( nextSamples ) == 0 )
				return null;
			currentSample -= samples.length;
		}
		
		//What does tempSamples do?
		//I think it has to do with hop size.
		...
		//currentSample is the number of total samples read, it's a marking in the entire PCM array of the song.
		//This is where the currentSample is first added. 
